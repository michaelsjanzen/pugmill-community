...
- **Enhanced:** When the plugin can benefit from AI (generation, classification, suggestions), those capabilities are additive -- they appear when AI is configured and are absent otherwise. The plugin must not fail or degrade core behaviour when AI is unavailable.
...
- **Hook names** must come from `ActionCatalogue` / `FilterCatalogue` in `hook-catalogue.ts`. Proposing a new hook means adding it to the catalogue and updating the relevant core call site.
